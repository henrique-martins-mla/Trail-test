...
# remove the default background transparency
...
def evaluate(
        y_true_class, y_pred_score, num_in_target=None, cut_prob=None,
        round_evals=True):
    """
    ...
    # [0] # samples that truly are in the target
    assert (tp + fn) == y_true_class.sum()
    ...
    b = 1
    F_1 = (1 + b*b) * precision * recall / (b*b * precision + recall)
    b = 2
    F_2 = (1 + b*b) * precision * recall / (b*b * precision + recall)
    ...
    evals = (
        ...
        ('precision', precision),
        ('recall', recall),
        ('TNR_neg. recall', specificity),
        ('NPV_neg. precis.', NPV),
        ('FOR_default rate', FOR),
        ('FOR_lift', FOR_lift),
        ('F1', F_1),
        ('F2', F_2),
        ('accuracy', accuracy),
        ...
        ('nr_in_target tp_plus_fp', tp + fp),
        ('true_targets tp_plus_fn', tp + fn),
...
data_dict = data_dict.set_index(
    'Variable Name').loc[list(column_mapping.values())]
    ...
    with mlflow.start_run(run_name=run_name, nested=True) as run:
        with Trail(experiment_title=run_name):
            model = LGBMClassifier(
                ...
                reg_lambda=reg_lambda, random_state=42, n_jobs=-1)
            # Train set metrics
            model.fit(X, y_true, verbose=10)

            # region explainability with shap
            shap_path = "model/model_explanations_shap"
            ...
            for name in X.columns:
                ...
                mlflow.log_figure(
                    pl.gcf(),
                    f"{shap_path}/dependence_plot_{name}.png",
                    save_kwargs={"bbox_inches": 'tight'})
                pl.close()

            # watterfall for first prediction
            shap.plots._waterfall.waterfall_legacy(
                explainer.expected_value, shap_values[1],
                feature_names=X.columns)
            mlflow.log_figure(
                pl.gcf(),
                f"{shap_path}/waterfall_plot_prediction_0.png",
                save_kwargs={"bbox_inches": 'tight'})
            pl.close()
            ...
            mlflow.log_figure(
                pl.gcf(),
                f"{shap_path}/force_plot_prediction_0.png",
                save_kwargs={"bbox_inches": 'tight'})
            pl.close()
            ...
            ev = evaluate(
                y_true, pd.Series(yp),
                num_in_target=y_true.mean())  # ev

            for metric, value in ev.items():
                mlflow.log_metric(metric, value)

            # register model
            mlflow.register_model(
                f"runs:/{run.info.run_id}/model", "staging.trail")
...
#     with Trail(experiment_title=run_name):
#         model = LGBMClassifier(
#             learning_rate=0.3, n_estimators=20, reg_lambda=1.0, random_state=42,
#             n_jobs=-1)
#         # Train set metrics
#         model.fit(X, y_true, verbose=10)
